...
public class MathUtil {
	...
	public static void splitRedPacket(int total, int splitCount, int min, int max) {
		...
		// System.out.println("将" + total + "元拆分为" + couples + "对金额，每对总额：" +
		// perCoupleSum);
	...
	public static void redPackage_02() {
		...
		int people = 10;
		for (int i = 0; i < 10; i++) {
			// System.out.println("the number" + people + "can get " + num / people *
			// N);
	...
	private float randomRedPacket(float money, float mins, float maxs, int count) {
		float result = 0;
		if (count == 1) {
			result = (float) (Math.round(money * 100)) / 100;
		} else if (mins == maxs) {
			result = mins;// 如果最大值和最小值一样，就返回mins
		} else {
